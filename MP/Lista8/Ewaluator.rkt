...
(define (cxxxr-cons id expr)
  (list 'cxxxr id expr))

(define (cxxxr-id->lst id)
...
;; lambda - one list argument (marg -> many arguments)

(define (lambda-marg? e)
  (and (tagged-tuple? 'lambda-marg 3 e)
       (symbol? (cadr e))))

(define (lambda-marg-cons sym expr)
  (list 'lambda-marg sym expr))

(define (lambda-marg-expr e)
  (caddr e))

(define (lambda-marg-var e)
  (cadr e))
...
(define (closure-marg-cons arg expr env)
  (list 'closure-marg arg expr env))

(define (closure-marg-arg c)
  (cadr c))

(define (closure-marg-expr c)
  (caddr c))

(define (closure-marg-env c)
  (cadddr c))

(define (closure-marg? c)
  (and (tagged-tuple? 'closure-marg 4 c)
       (symbol? (closure-marg-arg c))))
  ...
  (cond [(const? e) e]
        ...
        [(lambda-rec? e)
         (closure-rec-cons (lambda-rec-name e)
                           ...
                           env)]
        [(lambda-marg? e)
         (closure-marg-cons (lambda-marg-var e)
                            (lambda-marg-expr e)
                            env)]
        [(app? e)
         (apply-closure
...
(eval-env '((lambda-rec (append xs ys)
             (if (null? xs)
                 ys
...
(eval-env '((lambda-rec (map proc list)
                        (if (null? list)
                            null
                            (cons (proc (car list))
                                  (map proc (cdr list)))))
...
(flatten (eval-env '((lambda-rec (reverse list)
                        (if (null? (cdr list))
                            (car list)
...
(eval-env '(( lambda-marg x (+ ( car x ) ( car x ) ) ) 1 2 3 4 5 6) null)

(define (cxxxr id expr)
    (cond [(eq? (car id) 'r) (cxxxr (cdr id) expr)]
          [(eq? (car id) 'a) (cxxxr (cdr id) (car expr))]
          [(eq? (car id) 'd) (cxxxr (cdr id) (cdr expr))]
          [(eq? (car id) 'c) expr]
          [else (error "cxxxr: Bad syntax")]))

...
;; snd-application structure (list instead of cons)

(define (snd-app-cons proc arg)
  (list proc arg))

#| ARITH-EXPRESSION LANGUAGE

every arith-expr can be build by:
1. number (which satisfies number?? predicate)
2. (O number number) O ->(+ - * /)
3. (O number arith-expr)
4. (O arith-expr number)
5. (O arith-expr arith-expr)

expressions are lists (cons a1 (cons a2 (... (cons an null)...)
if ai (i>0 && i<=n) is O (operator) it must be quoted (e.g. (cons '+ (cons 2 (cons 3 null))))

every expression built differently has bad syntax
...
(define (arith-eval test)
  (snd-app-cons '(lambda-rec (eval e)
                             (if (pair? e)
                                 (cond [(eq? (car e) '+)
                                       (+ (eval (car (cdr e)))
                                          (eval (car (cdr (cdr e)))))]

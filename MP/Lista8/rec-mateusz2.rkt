...
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(and (eq? 'lazy (lazy-def-tag (car env)))
              (eq? x (lazy-def-var (car env))))
         (eval-env (lazy-def-val (car env)) env)]
        [(eq? x (caar env)) (cadar env)]
        [else (find-in-env x (cdr env))]))

;; lazy-def (new structure in environment)

(define (lazy-def? e)
  (tagged-tuple? 'lazy 3 e))

(define (lazy-def-tag e)
  (car e))
...
(define (env-for-l-let def env)
  (add-to-l-env
   (let-def-var def)
   (let-def-expr def)
   env))

(define (add-to-l-env var val env)
  (cons (lazy-def-cons var val) env))
